...
function unhighlightKey(freq) {
    const freqStr = freq.toFixed(2);
    if (typeof isNoteHeld === 'function' && isNoteHeld(freqStr)) return;
    const keys = getCachedKeys(freqStr);
...
function renderBoard() {
    ...
    for (let r = 0; r < currentRows; r++) {
        ...
        for (let c = 0; c < currentCols; c++) {
            ...
            const keyId = 'mouse_' + (keyCode || (isRightSide ? 'R_' : 'L_') + freqStr);

            // Upgraded Mouse Events (Touch is now handled globally)
            key.addEventListener("mousedown", (e) => {
                if (e.button !== 0) return;
                if (typeof pressNote === 'function') pressNote(freq, false, key, null, null, keyId);
            });
            key.addEventListener("mouseenter", () => {
                if (window.isMouseDown && typeof pressNote === 'function') pressNote(freq, false, key, null, null, keyId);
            });
            key.addEventListener("mouseleave", () => {
                if (typeof releaseNote === 'function') releaseNote(freq, false, key, null, null, keyId);
            });
...
function renderTraditionalPiano() {
    ...
    for (let i = 0; i < totalNotes; i++) {
        ...
        const keyId = 'mouse_strip_' + freqStr;
        ...
        key.addEventListener("mouseenter", () => {
            if (window.isMouseDown) pressNote(freq, false, key, null, null, keyId);
        });
        key.addEventListener("mouseleave", () => releaseNote(freq, false, key, null, null, keyId));
        key.addEventListener("mouseup", (e) => {
            if (e.button !== 0) return;
            releaseNote(freq, false, key, null, null, keyId);
        });
